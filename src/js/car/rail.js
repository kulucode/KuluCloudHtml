...
CarDrawerManager.prototype.select = function(carId) {
  /**changed the carId*/
  _this = this;
  // _this.map.centerAndZoom(_this.car.point, 17);
  //     _this.map.centerAndZoom(_this.car.fence[0],17);
  // _this.map.panTo(points[0]);
};
CarDrawerManager.prototype.selectAll = function() {
  //多选
  /**changed the carId*/
  _this = this;
  _this.ids = [];
...
CarDrawerManager.prototype.save = function() {
  ...
  if (len < 1) {
      ...
      function(isConfirm) {
        if (isConfirm) {
          ...
          if (_this.mode == "f") {
            ...
            RemoteApi.setTraits(_this.car.id, "", function() {
    ...
    if (_this.mode == "f") {
      ...
      RemoteApi.setTraits(_this.car.id, overlay.so, function() {
...
CarDrawerManager.prototype.drawFenceOrTraits = function(mode) {
  ...
  if (_this.car != null) {
    RemoteApi.getCar(_this.car.id, function(response) {
      //todo
      if (
        response.car.preset_traits != null &&
        response.car.preset_traits.length > 1
      ) {
        _this.car.preset_traits = JSON.parse(response.car.preset_traits);
      } else {
        _this.car.preset_traits = [];
        _this.car.preset_traits.length = 0;
      }

      if (response.car.fence != null && response.car.fence.length > 1) {
        _this.car.fence = JSON.parse(response.car.fence);
      } else {
        _this.car.fence = [];
        _this.car.fence.length = 0;
      }

      if (_this.mode == "f") {
        data = _this.car.fence;
      } else {
        data = _this.car.preset_traits;
      }
...
function echartsInit() {
  ...
  map.removeOverlay();
  overlay.length = 0;
  doRefresh("layuiPlate", "KULUINTERFACE", "searchFanceList", "&pg_type=1", function(data) {
    console.log(data);
    if (data.code == 0) {
      //执行正确动作
...
$(".search_Carlists").click(function(){
  ...
  echartsInit();
  doRefresh("layuiPlate", "KULUINTERFACE", "searchTruckWordParasList", "", function(
    data
  ) {
    if (data.code == 0) {
        console.log(data)
      ...
      car.initCar();
      showInfoWindow(data.data[0],map,car);
