    ...
    car.loadTrace = function () {
      ...
      doRefresh("carForm", "KULUINTERFACE", "getTruckTracePointList", "", function (data) {
        ...
        if (data.data.length > 0) {
          ...
          var firstPoint = new BMap.Point(data.data[0].bdlon, data.data[0].bdlat);
    ...
    car.startRoute = function (isPause, runnum, currentCount) {
      ...
      function resetMkPoint() {
        if (car.curPointIndex < numPoints) {
          ...
          var content =
            " 车牌号：" + pts[car.curPointIndex].platenum +
            " <br/>项目组：" + pts[car.curPointIndex].truckorg +
            " <br/>位置：(" +
            pts[car.curPointIndex].bdlon +
            " : " +
            pts[car.curPointIndex].bdlat +
...
function websocketInit() {
  ...
  let sock = new MyWebSocket(url, function (event) {
    ...
    //let carId = event.data.carId;
    let car = cm.getCar(carInfo.carId);
    if (car != undefined)
      car.updatePos(carInfo.curLong, carInfo.curLat, carInfo.areAlerted);
  });
}
