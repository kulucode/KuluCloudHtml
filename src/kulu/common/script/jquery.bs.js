...
var vActionFile = thisFileDomain + "/TTService?curPageOperID=curFile&curTranscoding=true&curBSID=";
...
function getRefreshURL(myform, bsid, opname, _thisDomain, _session) {
...
function doRefreshFile(myform, bsid, opname, _paras, _pdiv, callfun, _thisDomain, _session) {
    ...
    if (fileObj.length > 0) {
        for (var j = 0; j < fileObj.length; j++) {
            var fObj = fileObj[j];
            for (var v = 0; v < fObj.files.length; v++) {
                fd.append(fObj.name + "_" + v, fObj.files[v]);
            }
        }
        // 准备上传对象
        var xhr = false;
