...
function addMapPoint(_data) {
    ...
    var _desc = "<div><strong>车辆：" + _oneD.truckname + "</strong></div>";
...
function getTruckMaps() {
    isGJ = false;
    TruckList = {};
    doRefresh("", "KULUINTERFACE", "searchTruckWordParasList", "&",
        function (_data) {
            if (_data.code == 0) {
                mapRest();
                //成功后发送登录成功消息
                ...
                for (var i = 0; i < _data.data.length; i++) {
                    ...
                    addMapPoint(_oneP);
                    _html += "<option value='" + _oneP.eqpid + "'>" + _oneP.truckname + "[" + _oneP.paltenum + "]</option>";
                }
